...
laya::JCIosFileSource* m_pRedistFileResource = nullptr;
...
-(void)destory
{
    ...
    [LayaVideoPlayer removeCurParentView];
    
    if(m_pGLKView)
    {
        [m_pGLKView.subviews makeObjectsPerformSelector:@selector(removeFromSuperview)];
        m_pGLKView = nil;
    }
    if(m_pWebView)
    {
        m_pWebView->m_pWebView = nil;
        [m_pWebView->m_pCloseButton removeFromSuperview];
        m_pWebView->m_pCloseButton = nil;
        m_pWebView = nil;
    }
    ...
    if (m_pRedistFileResource) {
        delete m_pRedistFileResource;
        m_pRedistFileResource = nullptr;
    }
    if (g_pIOSConchRuntime) {
        g_pIOSConchRuntime = nil;
    }
    [conchConfig DelInstance];
}
...
- (void)dealloc
{
    ...
    if(m_pEditBoxDelegate)
    {
        m_pEditBoxDelegate = nil;
    }
...
-(void) onGLReady:(int)p_nWidth height:(int)p_nHeight downloadThreadNum:(int)nDownloadThreadNum
{
    if( m_bEngineInited == false )
    {
        ...
        m_pRedistFileResource = new laya::JCIosFileSource();
        gResourcePath = sResourcePath;
        gResourcePath += "/";
        gAssetRootPath = gResourcePath + "/cache/";
        m_pRedistFileResource->Init( gResourcePath.c_str() );
        laya::JCConch::s_pAssetsFiles = m_pRedistFileResource;
